...
export default class ChatRoom extends Component {
  ...
  sendPushNotification = async (text) => {
    const theirToken = (this.state.receiverToken) ? this.state.receiverToken[0] : '';
  ...
  getToken = () => {
    firebase
      ...
      .where('email', '==', this.state.f_email)
      .get()
      .then (querySnapshot => {
        const results = []
        ...
        this.setState({ receiverToken: results.map(item => item.token) })
        console.log(results)
  ...
  componentDidMount() {
    ...
    this.unsubscribeUser = firebase.firestore().collection('chat_user').onSnapshot(this.getToken);
  }
  ...
  onSend = () => {
    ...
    this.sendPushNotification(text)
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
              <View style={{
                ...
                borderRadius: 20,
                  ...
                  <Text style={{fontSize:16,color:"#000" }}>{base64.atob(c_data.text)}</Text>
              </View>
